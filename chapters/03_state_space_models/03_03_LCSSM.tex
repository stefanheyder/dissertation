...
\begin{definition}[\gls{pgssm}]
    an \acrfull{pgssm} is a joint distribution for $(X,Y)$ where states $X$ follow \Cref{eq:glssm_states}, i.e. 
    \begin{align*}
...
\begin{definition}[\gls{egssm}]
    \label{def:egssm}
     an \acrfull{egssm} is an \acrshort{pgssm} where the conditional distribution of $Y_{t}$ given $X_{t}$ forms an exponential family with respect to a base measure $\mu_{t}$, i.e.
    $$
