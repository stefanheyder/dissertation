...
For the applications considered in this thesis the distribution of observations is never Gaussian --- see \Cref{sec:dessiderata} --- and  all we can hope for is that the data-generating mechanism is close enough to a Gaussian distribution that inferences made in an \acrshort{glssm} may carry over.
...
\begin{definition}[\gls{pgssm}]
    ...
    $$
    we say the \acrshort{pgssm} has a \textbf{linear signal}, similar to the treatment in \citep[Part II]{Durbin2012Time}.
\end{definition}

It is straightforward to check that an \acrshort{pgssm} is indeed an \acrshort{ssm}. 
...
\begin{definition}[\gls{egssm}]
    \label{def:egssm}
    A \acrfull{egssm} is a \acrshort{pgssm} where the conditional distribution of $Y_{t}$ given $X_{t}$ forms an exponential family with respect to a base measure $\mu_{t}$, i.e.
    $$
    ...
    $$
    for functions $h_{t}^{i}: \R \to \R, Z^{i}_{t}: \R \to \R, \eta^{i}_{t}: \R\to\R, T: \R\to\R$, $i = 1, \dots p$, we say the \gls{egssm} has a \textbf{linear signal}, similar to the treatment in \citep[Part II]{Durbin2012Time}. Notice that for \acrshortpl{egssm} with linear signal, we also require that the conditional marginals stem from the same exponential family.
\end{definition}
