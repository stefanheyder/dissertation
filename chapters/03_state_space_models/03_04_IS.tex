...
% MCMC intro
\gls{mcmc} \citep{Brooks2011Handbook} is a simulation technique that allows simulation of samples from a target distribution by constructing an ergodic Markov chain that has as its invariant distribution the desired distribution. 
...
Thus, these samples can be used in \gls{mcint} to estimate expectations of interest, though one has to be mindful of autocorrelation of these samples.
For standard variants of \gls{mcmc}, such as Metropolis-Hastings \gls{mcmc} or Hamiltonian Monte Carlo, one needs access to the density of the sought after distribution up to a constant to simulate a step in the Markov chain. While these methods are very general, in high dimensions, these are affected by the curse of dimensionality. 

% MCMC vs IS
Let us argue for our choice of using \gls{is} over \gls{mcmc} for estimating conditional expectations of the form $\E \left( f(X)|Y \right)$ for \glspl{pgssm}. For the models we consider in this thesis, the dimension of $X$ ($(n+1)m$) can become quite large, so \gls{mcmc} suffers from the aforementioned curse of dimensionality. \gls{is} can also suffer from this curse, especially if the proposal is far from the target. If, however, the proposal is close to the target, \gls{is} can perform surprisingly well, see e.g. \citep{Chopin2017Leave} where it is used as the gold standard method against which other methods are benchmarked. 

As \gls{is} is based on independent samples, it can be parallelized easily, whereas parallelizing \gls{mcmc} is more involved, using e.g. \citep{Neiswanger2014Asymptotically}. Additionally, analysis of convergence is much simpler than that of \gls{mcmc}, which requires consideration of burn-in samples, autocorrelation of samples and investigating trace plots for the chain getting stuck. 
...
% SMC vs. IS
Conveniently, \gls{smc} allows us to approximate the likelihood $\ell(\theta)$ for a single parameter by a single pass of the particle filter. However, the discrete nature of resampling makes the approximated likelihood non-continuous, complicating maximum likelihood inference. \citet[Chapter 14]{Chopin2020Introduction} discuss several strategies: the first amounts to the same strategy employed in this thesis, where one fixes a reference parameter $\theta_{0}$ to perform importance sampling with $p_{\theta_{0}}(x|y)$ against $p_{\theta}(x|y)$. The second strategy only works in the univariate case and consists of approximating the non-continuous inverse CDFs appearing in the resampling step by continuous ones. Finally, if the dependence on the hyperparameters $\theta$ allows for application of the EM-algorithm, it may be used to perform the optimization.
...
To perform importance sampling one must be able to evaluate $w$. In the context of \glspl{pgssm} this is usually not possible: if $\P$ is the intractable conditional distribution of $X|Y$, then the integration constant of its density $p(y)$ is not analytically available.
...
% introduces bias
Unless $\tilde w$ is degenerate, i.e. constant, $$\hat\zeta = \frac{\sum_{i = 1}^N \tilde w (X^{i} f(X^{i}))}{\sum_{i = 1}^{N} \tilde w(X^{i})}$$ is a ratio of two non-constant, unbiased estimators and so is itself biased. Nevertheless, noticing that the rescaled denominator $\frac{1}{N} \sum_{i = 1}^{N} \tilde w(X^{i})$ consistently estimates the integration constant $\G \tilde w$, we may apply Slutsky's lemma and obtain a central limit theorem for $\hat \zeta$ (recall that we assumed $fw \in L^{2}(\G)$).

The class for test functions $f$ for which this holds depends on $\P$ and $\G$. \citet{Agapiou2017Importance} study the behavior of uniformly bounded test functions $\lVert f \rVert \leq 1$. For these functions it suffices that $w \in L^{2}(\G)$ to ensure asymptotic normality of $\zeta$. 
...
$$
the second moment of the importance sampling weights. \citet{Agapiou2017Importance} show that the bias 
\begin{align}
    \label{eq:agapiou_bias}
\left| \mathbb E (\hat\P_{N} - \P)[f] \right|
\end{align}
and \acrfull{mse}
\begin{align}
    \label{eq:agapiou_mse}
\mathbb E \left( (\hat \P_{N} - \P) [f] \right)^{2}
\end{align}
of importance sampling are both, for bounded $f$, of order $\mathcal O \left(\frac{\rho}{N}\right)$. Here the expectation $\mathbb E$ is with respect to the random particles $X^{1}, \dots, X^{N}$. Consequently, for bounded functions, keeping $ \frac{\rho}{N}$ small produces importance sampling estimates with small bias and \gls{mse}. This can be achieved in two ways: either we choose $\G$ \glqq{}close enough\grqq{} to $\P$ to ensure small $\rho$, or we choose $N$ large enough to compensate for a large $\rho$.

Applying Jensen's inequality, we see that
$$
\Dkl{\P}{\G} = \P [\log w] \leq \log \P[w] = \log\rho,
$$
so small $\rho$ implies a small \gls{kld} between $\P$ and $\G$ as well. Conversely, the following theorem of \citeauthor{Chatterjee2018Sample} implies that a small \gls{kld} is both sufficient and necessary for importance sampling to perform well.
\begin{theorem}[{\cite[Theorem 1.1]{Chatterjee2018Sample}}]
    \label{thm:chatterje2018Thm1}
     Let $\P$ and $\G$ be probability measures on a measurable space $(\mathcal X, \mathcal B)$ such that $\P \ll \G$ and let $f \in \mathbf L^2(\P)$ be a function and denote by $\lVert f \rVert_{L^{2}(\P)} = \left( \P [f^{2}] \right)^{1 / 2} < 
     \infty$ its $L^{2}(\P)$-norm. Let $Z$ be an $\mathcal X$ valued random variable with law $\P$. 
     
     Let $L = \Dkl{\P}{\G} = \mathbb E \log w(Z)$ be the \gls{kld} between $\P$ and $\G$, and let $$\hat \P_N = \sum_{i = 1}^N w(X^i) \delta_{X^i}$$ be the particle approximation of $\P$  based on samples $X^1, \dots, X^N\iid \G$, $N \in \N$. 
     
     If the sample size $N$ is given by $N = \exp\left( L + t \right)$ where $t \geq 0$,
     \begin{align} 
        \label{eq:chatterje-upper-bound}
        \mathbb E \left\lvert \hat \P_N[f] - \P [f] \right\rvert \leq \lVert f \rVert_{L^2(\P)} \left(\exp(-t / 4) + 2 \sqrt{\mathbb P \left( \log w(Z) > L + t / 2 \right)}\right). 
    \end{align}
...
\end{theorem}
Notice the boldface $\mathbb P$ and $\mathbb E$ to differentiate the measures $\P$ and $\G$ from expectations and probabilities with respect to the abstract probability space $\left( \Omega, \mathcal A, \mathbb P \right)$ where the random variables $X_{1}, \dots, X_{N}$ and $Z$ live.
Theorem 1.2 in the same paper provides a qualitatively similar result for autonormalised importance sampling.

However, there is room for improvement in \eqref{eq:chatterje-upper-bound}: as $t = \log N - L$, the first term of the right-hand side is $N^{-\frac{1}{4}} \exp(\frac{L}{4})$, a much slower rate of convergence than the usual $N^{-\frac{1}{2}}$ found in the law of large numbers. A small adaptation of the proof given in \citep{Chatterjee2018Sample} fixes this.

\begin{proposition}
    \label{prop:chatterje_improve}
    Under the conditions of \Cref{thm:chatterje2018Thm1} and for $\varepsilon \in (0,1)$ we have
    \begin{align}
        \label{eq:chatterje-upper-bound-eps}
        \mathbb E \left\lvert \hat \P_N[f] - \P [f] \right\rvert \leq \lVert f \rVert_{L^2(\P)} \left(\exp\left(- (1-\varepsilon)\frac{t}{2}\right) + 2 \sqrt{\mathbb P \left( \log w(Z) > L + \varepsilon t \right)}\right). 
    \end{align}
\end{proposition}

\begin{proof}
    We reiterate the proof of \eqref{eq:chatterje-upper-bound} as given in Section 4 of \citep{Chatterjee2018Sample}, changing only the cutoff parameter, $a$, to incorporate $\varepsilon$. Let $a = \exp \left( L + \varepsilon t \right)$ and set $h = f \cdot \mathbf 1_{w^{-1}((-\infty, a])}$. By the triangle inequality,
    $$
        \mathbb E \lvert \hat\P_{N}[f] - \P[f] \rvert \leq \mathbb E \lvert \hat \P_{N}[f - h] \rvert + \mathbb E \lvert \hat \P_{N} [h] - \P [h] \rvert + \lvert \P[f-h]\rvert
    $$
    holds. Noticing that $f - h$ = $f \mathbf 1_{w^{-1}((a, \infty))}$ and applying the Cauchy-Schwarz inequality, the last term is bounded by $$\lVert f \rVert_{L^{2}(\P)} \lVert \mathbf 1_{w^{-1}((a, \infty))} \rVert_{L^{2}(\P)} = \lVert f \rVert_{L^{2}(\P)} \sqrt{\mathbf P ( \log w(Z) > L + \varepsilon t)}.$$
    For the first term, a similar argument applies: 
    $$
        \mathbb E \lvert \hat \P_{N}[f - h] \rvert = \mathbb E \lvert \frac{1}{N} \sum_{i = 1}^{N} (f- h)\left(X^{i}\right)\rvert \leq  \mathbb E \lvert (f - h) \left( X^{i} \right) \rvert \leq \lVert f \rVert_{L^{2}(\P)} \sqrt{\mathbf P (\log w(Z) > L + \varepsilon t)}.
    $$

    To bound the middle term, as $\mathbb E \hat \P_{N}[h] = \P[h]$, we have
    \begin{align*}
        \mathbb E \lvert \hat \P_{N}[h] - \P[h] \rvert &\leq \sqrt{\var \left( \hat \P_{N}(h) \right)} = \sqrt{\frac{\var \left(w(X^{1})h(X^{1})\right)}{N}} \\
        &\leq \sqrt{\frac{\mathbb E \left(w(X^{1})^{2}h(X^{1})^{2}\right)}{N}} \leq \sqrt{\frac{a\mathbb E \left(w(X^{1})f(X^{1})^{2}\right)}{N}} = \sqrt{\frac{a}{N}} \lVert f \rVert _{L^{2}(\P)},
    \end{align*}
    as $h^{2} = f^{2} \mathbf 1_{w^{-1}((-\infty, a])}$ and so $\mathbb E \left(w(X^{1})^{2}h(X^{1})^{2}\right) \leq a \mathbb E \left(w(X^{1})f(X^{1})^{2}\right)$.
    Noticing that $ \frac{a}{N} = \exp(-(1-\varepsilon) t)$ and putting everything together, we obtain \eqref{eq:chatterje-upper-bound-eps}.
\end{proof}

With this slight adaptation, the first term in the upper bound is now $N^{-\frac{1-\varepsilon}{2}}$, and so can be made arbitrarily close to $N^{-\frac{1}{2}}$. 

Let us consider the implications of \Cref{thm:chatterje2018Thm1} and \Cref{prop:chatterje_improve}, starting with \eqref{eq:chatterje-upper-bound-eps}, by devising heuristics to decide when $\G$ is a good proposal for fixed sample size $N$, and assume for simplicity that $ \lVert f \rVert_{L^{2}(\P)} = 1 $.

First, as $t = \log N - L$, we have $\exp(- (1-\varepsilon) \frac{t}{2}) =  \exp (L \frac{(1-\varepsilon)}{2})N^{-\tfrac{1 - \varepsilon}{2}}$, so for large $N$ this term becomes negligible, and the interesting term in inequality \eqref{eq:chatterje-upper-bound} is the second one. As $\E \log w(Z) = L$, this term is a tail probability, and we can use standard mass-concentration inequalities to analyze its behavior as $t$ (and so $N$) grows. Markov's inequality tells us that 
$$
\mathbb P \left( \log w(Z) > L + \varepsilon t \right) \leq \frac{L}{L + \varepsilon t} = \frac{1}{1 + \varepsilon\frac{\log N - L}{L}},
$$
so, as $L \to 0$, the upper bound goes to $0$. 

\todo{ausführlichere Diskussion}
...
In both upper bounds provided by the concentration inequalities, all else being equal, a smaller \gls{kld} will yield a tighter bound. However, in Chebyshev's inequality, the variance of log weights also plays a role, and will surely be different for different proposals.
...
\begin{proof}
    ...
    $$
    Then $\xi' = \xi + \mu$ is in $[a,b]$, and note that, unless $U = 0$ a.s., $\mathbb E \exp U = 1$ forces $a < 0 < b$. 
    Thus,
    $$
...
\begin{corollary}
    ...
    Let $\P$ and $\G$ be equivalent probability measures with bounded Radon-Nikodym derivative $w = \frac{\d\P}{\d\G} \in [a,b]$, $a, b \in \R$ and \gls{kld} $\Dkl{\P}{\G} =\P [\log w]$.
    ...
    $$
\end{corollary}
Under the assumptions of this corollary, we see that a small variance of the log-weights implies a small \gls{kld}, which in turn implies good importance sampling performance.

Let us now discuss the implications of \eqref{eq:chatterje-lower-bound}. We see that for large $s$, i.e. $N \ll \exp(L)$, the right-hand side is small, and so the probability that importance sampling fails for the constant function is practically relevant. Observe that here 
$$
...
Based on this discussion, we see that choosing $\G$ such that either the \gls{kld} or the variance of the log-weights is small is sensible. Making the variance small has the additional advantage that it, at least for bounded log-weights, also implies an upper bound for the \gls{kld}. We will return to this train of thought when we discuss optimal ways of performing importance sampling, such as the \acem (minimizing the \gls{kld}) and \aeis (minimizing the variance of log-weights) in the following sub-chapters.
...
$$
Notice that as the self-normalized weights sum to $1$, the \gls{ess} is at least $1$, as $0 \leq W_{i} \leq 1$ and at most $N$ by the Cauchy-Schwarz inequality. 

If we write the \gls{ess} in terms of the unnormalized weights $\tilde w$ we see that the \gls{ef} $\text{EF} = \frac{\text{ESS}}{N}$ fulfills, as $N\to\infty$,
\begin{align}
...
\end{align}
if $\tilde w \in L^{2}(\G)$ \citep[Section 2.3.2]{Agapiou2017Importance}. We will call $\frac{1}{\rho^{-1}}$ the asymptotic efficiency factor. Thus, asymptotically, a large \gls{ess} leads to small bias and \gls{mse} for bounded functions $f$. Additionally, the above derivations allow us to interpret the second moment
$$
...
$$
as the asymptotic relative efficiency of the two estimators, as long as this approximation is valid. In practice, a small \gls{ess} can be an indicator that importance sampling with $\G$ may be inadequate. Note that relying solely on the empirical \gls{ess} may lead to problems, see the following example. To prepare, we prove a lemma regarding $\rho$ for Gaussian targets and proposals.

\begin{lemma}
    \label{lem:gaussian_proposal_factor_2}
    Let $\P = \mathcal N(\mu, \Sigma)$ and $\G = \mathcal N(\nu, \Omega)$ be two $p$-dimensional Gaussian distributions with means $\mu,\nu \in \R^{p}$ and \gls{spd} covariance matrices $\Sigma,\Omega \in \R^{p\times p}$. 
...
\begin{example}[failure of the \gls{ess}]
    ...
    $$
    is finite if, and only if, $\varepsilon^{2} > \frac{1}{2}$, by the preceding lemma. Thus, for $\varepsilon^{2} \leq \frac{1}{2}$ interpreting the \gls{ess} or \gls{ef} is not sensible. Nevertheless, given samples $X^{1}, \dots, X^{N} \iid \G$, we may calculate the \gls{ess} in the usual way. If $N$ is only moderately large, there is a high probability that most samples do not lie in a region where weights are small, i.e. in the tails of the second component. Thus, unless $N$ is large, the empirical \gls{ess} will be large, deceiving us to think that importance sampling with $\G$ is feasible.

    We illustrate this by a simulation study, where we calculate the \gls{ef} $M=100$ times for different values of $N$ and $\varepsilon$. We used $N = 100, 1\,000, 10\,000$ and $\varepsilon^{2} = 0.01, 0.1, 0.5$; the results may be found in \Cref{fig:ess_failure}. Notice that for all values of $\varepsilon$ considered, we have $\rho = \infty$. We see that even for $N = 1\,000$ and $\varepsilon = \frac{1}{2}$ the upper quartile of \glspl{ef} is $71\%$, which seems reasonable to declare importance sampling to perform well. 

    Let us note that having access to the normalized weights $w$ here allows us to spot this deficiency of the \gls{ess} by recognizing that while \gls{ess} is high, the weights $w$ are not close to $1$, but rather $\frac{1}{2}$.
...
\end{align}
which goes to $\infty$ as $\sigma^2$ does. To show \eqref{eq:variance-wxx} we calculate the second moment of $w(X)X$,
\begin{align*}
    ...
    &= \nu \sigma \int x^{2} \frac{1}{\sqrt{2\pi\nu^{2}}} \exp \left( - \frac{x^{2}}{2\nu^{2}}\right)\d x \\
    &= \nu^{3} \sigma = \frac{\sigma^{4}}{(2\sigma^{2} - 1)^{\frac{3}{2}}}
\end{align*}
where $\nu^{2} = \frac{\sigma^{2}}{2\sigma^{2} - 1}$.

Thus, for a pre-specified $\epsilon > 0$, let $N$ be as in \Cref{thm:variance_failure} and choose $\sigma^{2}$ such that $\var \left( \hat\zeta_{N} \right) = \frac {\tau^{2}}N$ is larger than, say, $10\epsilon$. By the preceding theorem, we would, with large probability, observe a small empirical variance and thus declare $\hat\zeta_{N}$ to have converged, whereas, in reality, we would need a sample size that is $100$ times as large.
...
In the following sections, we will predominantly take the position that we are interested in finding a good particle approximation $\hat\P_{N}$ of the form \eqref{eq:is-particle-approximation} over finding the optimal proposal $\G^{\ast}$ from \Cref{prop:minimum_MSE_IS} and assume that the importance sampling weights can only be evaluated up to a constant. 
This has several reasons: First of all, for most problems considered in this thesis $\P$ is usually a conditional distribution, e.g. $\P = \mathbb P^{X|Y=y}$ for states $X$ and observations $Y$ in the \gls{ssm} context. Should the appropriate densities exist, evaluating the weights amounts to calculating 
$$
