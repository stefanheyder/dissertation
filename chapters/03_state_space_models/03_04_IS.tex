...
However, there is room for improvement in \Cref{eq:chatterje-upper-bound}: as $t = \log N - L$, the first term of the right-hand side is $N^{-\frac{1}{4}} \exp(\frac{L}{4})$, a much slower rate of convergence than the usual $N^{-\frac{1}{2}}$ found in the law of large numbers. A small adaptation of the proof given in \citep{Chatterjee2018Sample} fixes this.
...
\begin{proof}
    We reiterate the proof of \Cref{eq:chatterje-upper-bound} as given in Section 4 of \citep{Chatterjee2018Sample}, changing only the cutoff parameter, $a$, to incorporate $\varepsilon$. Let $a = \exp \left( L + \varepsilon t \right)$ and set $h = f \cdot \mathbf 1_{w^{-1}((-\infty, a])}$. By the triangle inequality,
    $$
    ...
    Noticing that $ \frac{a}{N} = \exp(-(1-\varepsilon) t)$ and putting everything together, we obtain \Cref{eq:chatterje-upper-bound-eps}.
\end{proof}
...
Let us consider the implications of \Cref{thm:chatterje2018Thm1} and \Cref{prop:chatterje_improve}, starting with \Cref{eq:chatterje-upper-bound-eps}, by devising heuristics to decide when $\G$ is a good proposal for fixed sample size $N$, and assume for simplicity that $ \lVert f \rVert_{L^{2}(\P)} = 1 $.
...
\end{align}
which goes to $\infty$ as $\sigma^2$ does. To show \Cref{eq:variance-wxx} we calculate the second moment of $w(X)X$,
\begin{align*}
