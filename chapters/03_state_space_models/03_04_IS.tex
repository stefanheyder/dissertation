...
\begin{theorem}[{\cite[Theorem 1.1]{Chatterjee2018Sample}}]
    \label{thm:chatterje2018Thm1}
     ...
     Let $L = \Dkl{\P}{\G} = \mathbb E \log w(Z)$ be the \acrshort{kld} between $\P$ and $\G$, and let $$\hat \P_N = \sum_{i = 1}^N w(X^i) \delta_{X^i}$$ be the particle approximations of $\P$  based on samples $X^1, \dots, X^N\iid \G$, $N \in \N$. 
...
\begin{proposition}
    ...
    \begin{align}
        ...
        \mathbb E \left\lvert \hat \P_N[f] - \P [f] \right\rvert \leq \lVert f \rVert_{L^2(\P)} \left(\exp(- (1-\varepsilon)\frac{t}{2}) + 2 \sqrt{\mathbb P \left( \log w(Z) > L + \varepsilon t \right)}\right). 
    \end{align}
\end{proposition}

\begin{proof}
    We reiterate the proof of \Cref{eq:chatterje-upper-bound} as given in Section 4 of \citep{Chatterjee2018Sample}, changing only $a$ to incorporate $\varepsilon$. Let $a = \exp \left( L + \varepsilon t \right)$ and set $h = f \cdot \mathbf 1_{w \leq a}$. By the triangle inequality,
    $$
    ...
    $$
    holds. Noticing that $f - h$ = $f \mathbf 1_{w > a}$ and applying the Cauchy-Schwarz inequality, the last term is bounded by $$\lVert f \rVert_{L^{2}(\P)} \lVert \mathbf 1_{w > a} \rVert_{L^{2}(\P)} = \lVert f \rVert_{L^{2}(\P)} \sqrt{\mathbf P ( \log w(Z) > L + \varepsilon t)}.$$
    For the first term, the same argument applies, it is also bounded by $\lVert f \rVert_{L^{2}(\P)} \sqrt{\mathbf P (\log w(Z) > L + \varepsilon t)}$.
...
%The proof of this theorem is based on splitting $\mathcal X$ into $\{\log w \leq L + \frac{t}{2}\} $ and its complement and straightforward, it may be found in the Appendix of \citep{Chatterjee2018Sample}. 
