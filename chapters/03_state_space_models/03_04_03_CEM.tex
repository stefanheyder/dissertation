...
\begin{theorem}[\gls{clt} for $\hpce$]
    ...
    $$
    where $B = I(\pce) = \cov_{\G_{\pce}} (T)^{-1}$ and $$M = \cov_{\G} (w(T - \P [T])) = \G \left[w^{2} (T - \P[T])(T - \P[T])^{T}\right] = \P \left[ w (T - \P [T]) (T - \P [T])^{T} \right].$$
\end{theorem}
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item[(A)] \label{it:wbprimeL2} $w(\cdot) b'(\cdot, \psi, \xi) \in L^{2}(\G)$ in a neighborhood $N$ of $\psi^{\ast}$ for all $\xi \in \R^{k}$. %\todo{rewrite differentiable + second moment}
    \end{enumerate}
    ...
    \end{equation}
    where $M = \cov \left( w(X)\nabla_{\psi} b(X,\psi^{\ast}) \right)$ for $X\sim\G$ and $B= - \left(H_{\psi} \P \left[ b(\cdot, \psi^{\ast}) \right]\right)^{-1}$ as in \Cref{thm:haberman-clt}. Additionally
    \begin{equation}
...
\begin{proof}
    ...
    exist and is non-singular. Similarly, it is obvious that $\tilde b'(x,\psi,\xi) = w(x) b'(x, \psi, \xi)$ and so for $X\sim\G$ 
    $$
...
That $wT \in L^{2}(\G)$ is required for the application of \Cref{thm:haberman-clt} in the importance sampling setting, and can be seen as a usual regularity condition for such central limit theorems, involving now the weights $w$ as well, because of the importance sampling. In practice, this condition should not be problematic, as long as the initial proposal $\G$ is not too far from the target $\P$.
