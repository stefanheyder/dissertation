...
\label{subsec:cem}
Recall from our discussion surrounding \Cref{thm:chatterje2018Thm1} that a small \gls{kld} between the target $\P$ and the proposal $\G$ implies good performance for importance sampling. As the \gls{kld} depends on global properties of $\P$, i.e. the Radon-Nikodym derivative $\rnd{\P}{\G}$, minimizing it leads to a global approximation of $\P$, improving on the local-approximation provided by the \gls{la}.

The \gls{cem} \citep{Rubinstein1999CrossEntropy,Rubinstein2004CrossEntropy} implements this idea and selects from a parametric family $ \left( \G_{\psi} \right)_{\psi \in \Psi}$ of proposals the one that minimizes the \gls{kld} to the target. Here $\Psi$ is usually a subset of $\R^{k}$, which may be open, closed or neither. Thus, the \gls{cem} aims at solving the following optimization problem
$$
...
The \gls{kld} is given by 
$$
    \Dkl{\P}{\G_{\psi}} = \P \left[ \log w_{\psi} \right],
$$
and can be infinite, e.g. if $\P$ does not possess second moments and $\G_{\psi}$ are Gaussian distributions. If the \gls{kld} is infinite for all $\psi\in\Psi$, the \gls{cem} becomes uninteresting. As such we will require that the \gls{kld} is finite for at least one $\psi \in \Psi$, and restrict $\Psi$, without loss of generality, to those $\psi$ where the \gls{kld} is finite. 
...
As we have seen in \Cref{lem:log-concavity}, the densities of exponential families are log-concave in the natural parameter, and as such they will be the primary candidates for our investigations of the \gls{cem}. 
...
\begin{proposition}[The \gls{cem} for exponential families]
    ...
    \P[T] = \G_{\pce} [T],
    $$
    then $\pce$ is a maximizer of \eqref{eq:ce_argmax}. Furthermore, if $\cov_{\G_{\pce}} (T)$ is positive definite the maximizer is unique.
\end{proposition}
...
\end{proof}
As a consequence, the \gls{cem} for natural exponential families reduces to matching the moments of the sufficient statistic of the target and proposal.
...
While $\P [T]$ is usually not available, it is itself amenable to importance sampling. Given a helper proposal $\H$ we may estimate $\P [T]$ by $\hat\P_N [T] = \sum_{i = 1}^{N} W^{i} T(X^{i})$ for $X^{1}, \dots, X^{N} \iid \H$ and auto-normalized importance sampling weights $W^{i}$ and in turn, applying \Cref{prop:cem_exponential_families}, estimate $\psi_{\text{CE}}$ by $\hat \psi_{\text{CE}}$ solving
\begin{equation}
    \label{eq:ee_hpce}
\hat \P_N [T] = \G_{\hpce} [T].
\end{equation}
As $T \in L^{1}(\hat \P_{N})$, the only conditions we have to check to apply the above proposition are that this equation has a unique solution $\H$-almost surely in the interior of $\Psi$ and that $\Psi$ is convex. 

\begin{remark}[overview over measures used]
    \label{rem:measures_in_is}
    For the sake of clarity, let us summarize the measures used throughout this subsection.\footnote{We will use the same schema for the next subsection as well.}

    We start with $\P$, the \textbf{target measure}, which is intractable, but the main interest of our overall endeavor. As we do not have access to $\P$, our goal is to construct an \textbf{approximation} to it and use importance sampling to calculate the integrals of interest with respect to $\P$. To this end, we want to choose from an exponential family, $(\G_{\psi})_{\psi \in \Psi}$ the optimal proposal, as a solution to \eqref{eq:ce_argmax}\footnote{\eqref{eq:eis_optim} for \gls{eis}.}. Unfortunately, the objective function depends on $\P$ as well, and so is itself not tractable. We thus introduce a \textbf{helper measure}, $\H$, whose sole purpose is to approximately solve \eqref{eq:ce_argmax} using importance sampling. $\H$ may stem from the family of proposals $(\G_{\psi})_{\psi \in \Psi}$, but is not restricted to these measures. 

    Thus, $\H$ and $\G_{\pce}$ serve different purposes: $\H$ is used to facilitate the search for the optimal $\pce$, while $\G_{\pce}$ is the resulting approximation of $\P$.
\end{remark}


To apply the \gls{cem} in practice, one usually iterates the sampling and estimation steps, using the previously found $\hpce$ to sample in the current iteration and starting the iteration with a proposal from the same exponential family $\H = \G_{\psi^{0}}$. To ensure numerical convergence, a popular device is that of \glspl{crn}, i.e. using the same random number seed in all iterations. A basic version of the \gls{cem} is presented in \Cref{alg:cem-basic}.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Repeat
            ...
            \State estimate $\hpce$ \Comment{\eqref{eq:ee_hpce}}
...
\glsreset{cod}
Importance sampling is well known to exhibit the \gls{cod} \citep{Bengtsson2008Curseofdimensionality}, i.e. the phenomenon that in many problems, unless $N$ grows exponentially with the dimension of $\mathcal X$, the weights collapse to a single particle, i.e. $W^{(N)} \to 1$ as the dimension of $\mathcal X$ goes to $\infty$. As the \gls{cem} employs importance sampling to obtain $\hpce$, it too is affected by this phenomenon, see also \Cref{sec:simulation_studies}. 
%% screening method
The screening method \citep{Rubinstein2009How} deals with the \gls{cod} by keeping components of $\psi^{l}$ that vary too much from iteration to iteration fixed, in essence reducing the dimension of $\Psi$. 
Alternatively, the improved cross-entropy method \citep{Chan2012Improved} suggests generating approximately independent samples from $\P$ by, e.g., \gls{mcmc}-methods, and replacing the importance sampling version of $\hat\P_{N}$  in \eqref{eq:ee_hpce} by the actual empirical distribution.
Still, in high dimensions both of these approaches may be difficult to implement: the screening method may not move far from the initial proposal and \gls{mcmc}-methods are expensive in high dimensions.


As stated in \citep{Chan2012Improved} there may be two reasons as to why the \gls{cem} fails: either the parametric family is not rich enough to give a good approximation to $\P$, i.e. $ \Dkl{\P}{\G_{\pce}}$ is still large, or the estimate $\hpce$ fails to be close to $\pce$. As our simulation studies in \Cref{sec:simulation_studies} suggest, the reason for the degeneracy seems to be the latter. It will thus be beneficial to investigate the asymptotic behavior of $\hpce$. 

In the remainder of this section, we will derive novel results on the performance of the estimator $\hpce$ of $\pce$. In particular, we will investigate under which conditions $\hpce$ is consistent and asymptotically normal. To focus on the asymptotic behavior, we will only perform a single iteration of the basic \gls{cem} algorithm (\Cref{alg:cem-basic}). While we restrict ourselves here to the setting of $k$-dimensional natural exponential families, these results should generalize to other classes of distributions as well. The advantage that this class of families has is that due to the structure of the densities, they provide straightforward (regularity) conditions for the asymptotic results to hold. As the target functions are concave, these conditions are rather liberal. We start with proving the consistency of $\hpce$. 

\begin{theorem}[consistency of $\hpce$]
    \label{thm:cem-consistent}
    Adopt the same assumptions as in \Cref{prop:cem_exponential_families}. Furthermore, let $\H \gg \P$ be a proposal distribution and assume that
    
    \begin{enumerate}
        \item $\pce$ is the unique maximizer of \eqref{eq:ce_argmax},
...
\begin{theorem}[{\citep[Theorem 5.1]{Haberman1989Concavity}}]
    ...
    $$ 
    Let $(X^{i})_{i \in \N} \iid \P$ be a sequence of i.i.d. random variables with distribution $\P$ and let for $N \in \N$ $$\hat \P_{N} = \frac{1}{N} \sum_{i = 1}^N \delta_{X^{i}}$$ be their empirical distribution. Let $\left(\hat\psi_{N}\right)_{N \in \N}$ be a sequence of M-estimators, i.e. a sequence of maximizers of
    $$
    \hat b_{\Psi}: \Psi \to [-\infty, \infty), \psi \mapsto \hat \P_{N} [b(\cdot, \psi)].
    $$
    Assume that the following conditions hold:\footnote{Note that while the actual theorem assumes conditions 1,2,5 and 6 in the paper, C3 as stated here implies conditions 5 and 6, see also the discussion in Sections 2.3 and 2.4 in \citep{Haberman1989Concavity}.}
    \begin{enumerate}[label=(C\arabic*),ref=(C\arabic*)]
        ...
        \item\label{it:C3} $\Psi$ is convex and $b_{\Psi}$ is finite on an open neighborhood of $\psi^{\ast}$.
    \end{enumerate}
...
\begin{proposition}
    ...
    Assume that the conditions of \Cref{thm:haberman-consistent} are fulfilled and let $\H \gg \P$ be another probability measure with Radon-Nikdoym derivative $w(x) = \rnd{\P}{\H}(x)$. 
    Let $(X^{i})_{i \in \N} \iid \H$ and consider the particle approximations 
    \begin{align*}
        \tilde \P_{N} &= \frac{1}{N}\sum_{i = 1}^N w(X^{i})\delta_{X^{i}},  \\ 
        \hat \P_{N} &= \sum_{i = 1}^{N} W^{i} \delta_{X^{i}},
    \end{align*}
    and suppose for every $N\in\N$ there a.s. exist M-estimators
    \begin{align*}
...
\begin{proof}
    ...
    $$
    Then $\H \left[ \tilde b(\cdot, \psi) \right] = \P \left[ b(\cdot, \psi) \right]$ for all $\psi\in\Psi$, and so $\psi^{\ast}$ is the unique global maximum of $$\psi \mapsto \H \left[ \tilde b(\cdot, \psi) \right].$$ 
    As $\H [\tilde b(\cdot, \psi)] = \P [b(\cdot, \psi)] < \infty$ and for fixed $x\in\mathcal X$ $\tilde b(x, \cdot) = w(x)b(x,\cdot)$ is concave, we may directly apply \Cref{thm:haberman-consistent} to $\tilde\psi_{N}$, showing its strong consistency.

    For $\hat\psi_{N}$, notice that for a fixed sample $X^{1}, \dots, X^{N} \iid \H$ and any function $f: \mathcal X \to [-\infty, \infty)$ we have, a.s., 
    $$
        \hat \P _{N} [f] = \sum_{i = 1}^{N} W^{i} f(X^{i}) = \frac{\H [\tilde w]}{\sum_{i = 1}^N \tilde w(X^{i})} \sum_{i = 1}^{N} \frac{\tilde w(X^{i})}{\H[\tilde w]} f(X^{i}) = \frac{\H [\tilde w]}{\sum_{i = 1}^{N} \tilde w(X^{i})} \tilde \P _{N} [f] \propto \tilde \P_{N} [f],
    $$
    where $\tilde w$ are the unnormalized weights, i.e. $ \frac{\tilde w(x)}{\H [\tilde w]} = w(x), x \in\mathcal X$.
    Thus $\hat \psi_{N}$ maximizes $\tilde \P_{N} [b(\cdot, \psi)]$ as well, and the result follows from the consistency of $\tilde \psi_{N}$.
\end{proof}

Let us now prove the promised consistency of the \gls{cem}.
...
$$
These rather strong properties enable us to derive a central limit theorem for the \gls{cem} with natural exponential family proposals under quite liberal conditions.

\begin{theorem}[\gls{clt} for $\hpce$]
    \label{thm:cem-clt}
    Adopt the same assumptions as in \Cref{prop:cem_exponential_families}. Furthermore, let $\H \gg \P$ be a proposal distribution with weights $w = \rnd{\P}{\H}$ and assume that
    \begin{enumerate}
        \item\label{it:pceinterior} $\pce \in \Psi$ is the unique maximizer of \eqref{eq:ce_argmax} which lies in the interior of the convex parameter space $\Psi$, 
        \item\label{it:FIMspd} the Fisher information matrix $I(\pce)$ exists and is positive definite,
        \item\label{it:wTinL2G} $w,wT \in L^{2}(\H)$, and 
    ...
    $$
    where $B = I(\pce) = \cov_{\G_{\pce}} (T)^{-1}$ and $$M = \cov_{\H} (w(T - \P [T])) = \H \left[w^{2} (T - \P[T])(T - \P[T])^{T}\right] = \P \left[ w (T - \P [T]) (T - \P [T])^{T} \right].$$
\end{theorem}
...
\begin{theorem}[{\citep[Theorem 6.1]{Haberman1989Concavity}}]
    \label{thm:haberman-clt}
    Consider the same setting as in \Cref{thm:haberman-consistent}. 

    Assume further that $\psi^{\ast}$ lies in the interior of $\Psi$ and that the following conditions hold\footnote{Note, again, that the original theorem is based on conditions 7,8,9 in the paper. However, under \ref{it:C7}, condition \ref{it:C10} implies conditions 8 and 9 in the paper. See the discussion in Section 3.1 in \citep{Haberman1989Concavity}.}: 
    \begin{enumerate}[label=(C\arabic*),ref=(C\arabic*)]
        \setcounter{enumi}{6}
        ...
        \end{align*}
        where $b'(x, \psi, \xi) = \lim_{a \downarrow 0} a^{-1} \left( b(x, \psi + a\xi) - b(x, \psi)\right)$ is the directional derivative. Note that if $b$ is differentiable for all $\psi \in V$, $b'(x, \psi, \xi) = \xi^{T}\nabla_{\psi} b(x, \psi)$ and it suffices to assume $(\nabla_{\psi} b(x,\psi))_{i}(\nabla_{\psi} b(x,\psi))_{j} \in L^{1} (\P)$ for all $\psi \in V$ and $i,j = 1,\dots, k$.
    \end{enumerate}
...
Similar to the consistency result above (\Cref{prop:is-consistency}), we need to extend this \gls{clt} to account for importance sampling.
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item[(A)] \label{it:wbprimeL2} $w(\cdot) b'(\cdot, \psi, \xi) \in L^{2}(\H)$ in a neighborhood $N$ of $\psi^{\ast}$ for all $\xi \in \R^{k}$. %\todo{rewrite differentiable + second moment}
    \end{enumerate}
    ...
    \end{equation}
    where $M = \cov \left( w(X)\nabla_{\psi} b(X,\psi^{\ast}) \right)$ for $X\sim\H$ and $B= - \left(H_{\psi} \P \left[ b(\cdot, \psi^{\ast}) \right]\right)^{-1}$ as in \Cref{thm:haberman-clt}. Additionally
    \begin{equation}
...
\begin{proof}
    ...
    \tilde b(x,\psi) = w(x) b(x, \psi),
    $$
    and notice that $\H \left[ \tilde b(\cdot, \psi) \right] = \P \left[ b(\cdot, \psi) \right]$. Let us verify the conditions of \Cref{thm:haberman-clt} for $\tilde b$ and the probability measure $\H$. 

    For condition \ref{it:C7}, as $H_{\psi} \P [b(\cdot, \psi)]$ exists and is non-singular, so does $$H_{\psi} \H [\tilde b(\cdot, \psi)] = H_{\psi} \P [b(\cdot, \psi)]$$
    exist and is non-singular. Similarly, it is obvious that $\tilde b'(x,\psi,\xi) = w(x) b'(x, \psi, \xi)$ and so for $X\sim\H$ 
    $$
        \sigma_{\tilde b}^{2}(\psi, \xi) = \mathbb E \left(\tilde b'(X, \psi, \xi)\right)^{2} = \mathbb E w^{2}(X) b'(X, \psi, \xi)^{2} < \infty
    $$
    by assumption \ref{it:wbprimeL2}, showing condition \ref{it:C10}. Thus we may apply \Cref{thm:haberman-clt} to $\tilde b$ and $\H$, finishing the proof.
\end{proof}

Interestingly, importance sampling only affects the $M$ component of the asymptotic variance. The reason for this is that $M$ is a quadratic function of the weights $w$, while $B$ only depends linearly on $w$, allowing to switch integrators from $\H$ to $\P$.
...
\begin{proof}[Proof of \Cref{thm:cem-clt}]
    ...
    $$
        w(x)b'(x,\psi,\xi) = w(x) \xi^{T}\nabla_{\psi} b(x, \psi) = \xi^{T} \left( w(x) (T(x) - \G_{\psi}[T]) \right) \in L^{2} (\H)
    $$
    by assumption \ref{it:wTinL2G}.

    Finally, to show the representation of $M$, note that by \Cref{prop:is-clt} we have for $X \sim \H$
    $$
        M = \cov \left( w(X) \left( T(X) - \G_{\pce}[T]\right)\right) = \H [w^{2} (T - \P [T])(T - \P[T])^{T}],
    $$
    as $\G_{\pce} [T] = \P [T]$ by \Cref{prop:cem_exponential_families}, so $$\E w(X) \left( T(X) - \G_{\pce} [T] \right) = \H[w (T - \G_{\pce}[T])] = \P[T - \G_{\pce}[T]] =  0.$$
\end{proof}

The form of the asymptotic covariance matrix is that of the sandwich estimator \citep{White1982Maximum}, corrected for the importance sampling with $\H$. This is not surprising: the \gls{cem} essentially performs maximum likelihood estimation of $\psi$ where the data comes from the misspecified $\P$. Additionally, we have to correct the variance for performing importance sampling with $\H$, instead of sampling directly from $\P$.

% discuss assumptions of Thm
The assumptions of \Cref{thm:cem-clt} are natural to facilitate the proof. The existence and positive definiteness of the Fisher information matrix are easily checked for the exponential family proposal and hold for minimal regular exponential families. Additionally, we have two moment constraints that involve the weights $w$ and the sufficient statistic $T$. 

That $wT \in L^{2}(\H)$ is required for the application of \Cref{thm:haberman-clt} in the importance sampling setting, and can be seen as a usual regularity condition for such central limit theorems, involving now the weights $w$ as well, because of the importance sampling. In practice, this condition should not be problematic, as long as the initial proposal $\H$ is not too far from the target $\P$.
That $T\in L^{2}(\P)$ is required for the application of \Cref{thm:haberman-clt}, and, as mentioned before, should not be problematic in practice, except for heavy-tailed distributions. It is a natural condition, as $T \in L^{2}(\G_{\pce})$ (as long as $\pce$ lies in the interior of $\Psi$) and $\G_{\pce}$ is supposed to approximate $\P$ well.
...
% extension to non EF
If $(\G_\psi)_{\psi \in \Psi}$ does not form an exponential family, $\hat\psi_{\text{CE}}$ will still be consistent and asymptotically normal, provided the usual regularity conditions for M-estimators apply.
...
However, these conditions will become more intricate than the ones we have provided here, as the concavity of the log densities is a rather strong property. As a result, we expect that assessing whether these conditions are satisfied in practice is more difficult. 