...
While the Kalman-filter (\Cref{alg:kalman_filter}) allows analytical computation of this likelihood \glspl{glssm}, in general \glspl{ssm} it is numerically intractable. The reason for this is that
$$
...
Similarly, when proposing with an \gls{glssm} or Markov-proposal for an \gls{pgssm}, the weights have similar structure, see \eqref{eq:weights_markov,eq:weights_only_on_signal}, which makes calculation of $\tilde w$ efficient. 
...
Given an \gls{glssm}-proposal and samples $X^{1}, \dots, X^{N}$ from it, we can calculate these antithetic variables efficiently: for the location balanced antithetic we can calculate the mean using the Kalman-smoother and for the scale balanced antithetic we can calculate $c$ and $c'$ using the inverse CDF of the $\chi^{2}_{k}$ distribution and the standard normal samples used to sample $X^{i}$ in the first place, for which fast implementations are readily available. Incidental, we obtain a third antithetic, 
\begin{align}
...
Similarly, by considering $\mathfrak X^{2}$, we can estimate the conditional variance, and by considering $\mathbf 1 _{\mathfrak X \leq x}$, we may estimate the conditional \acrlong{cdf} of $\mathfrak X$ given $Y$ at $x$, which is just the empirical \gls{cdf} of samples $\mathfrak X^{i}$ with associated weights $W^{i}$, $i = 1,\dots, N$.
Consequently, we can estimate the $\alpha$-quantile of $\mathfrak X | Y$ by the $\alpha$-quantile of this empirical \gls{cdf}, where we use the standard convention for empirical quantiles of linear interpolation between samples to make quantiles unique, see also \citep[Section 11.5.3]{Durbin2012Time}.