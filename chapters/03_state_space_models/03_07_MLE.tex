...
While the Kalman-filter (\Cref{alg:kalman_filter}) allows analytical computation of this likelihood for \glspl{glssm}, for general \glspl{ssm} it is numerically intractable. The reason for this is that
$$
...
\begin{align}
   \label{eq:nabla_log_p}
   \nabla_{\theta} p_{\theta}(x,y) = p_{\theta}(x,y) \nabla_{\theta} \log p_{\theta}(x,y), 
\end{align}
we have, provided we can switch the order of integration and differentiation,
\begin{align*}
...
By the structure of the model, \eqref{eq:joint_density}, the log density and its gradient can be computed efficiently by
\begin{align*}
...
Similarly, when proposing with a \gls{glssm} or Markov-proposal for a \gls{pgssm}, the weights have similar structure, see \eqref{eq:weights_markov,eq:weights_only_on_signal}, which makes calculation of $\tilde w$ efficient. 

For the remainder of this section, let us consider the \gls{glssm}-proposal obtained by the \gls{la} or \gls{eis} for an \gls{pgssm} with linear signal, as this is the main setting of \Cref{cha:analysis_of_selected_models}. For this we obtain 
$$
...
\end{align}
Notice that $\log g_{\theta}(z)$ is the likelihood in an \gls{glssm}, which can be computed efficiently by the standard Kalman filter (\Cref{alg:kalman_filter}). As in the \gls{glssm}-approach we propose with  an \gls{glssm} whose state density $g(x)$ and observation matrices $B_{t}$, $t = 0, \dots, n$ are equal to those of the target, the log-likelihood $\log g_{\theta}(z)$ also depends on $\theta$. The estimated gradient of the log-likelihood is 
$$
    \verywidehat{\nabla_{\theta} \log p_{\theta}(y)} = \nabla_{\theta} \log g_{\theta}(z) + \sum_{i=1}^N W^{i}_\theta \sum_{t = 0}^n \nabla_{\theta} \log p_{\theta}(y_{t}|S_{t}^{i}).
$$
The gradient of the \gls{glssm} log-likelihood can be obtained either numerically or analytically by employing the Kalman filter and smoother \citep{Koopman1992Exact}, however, numerical evaluation may be faster if the dimension of $\theta$ is small compared to the length of the time series, as evaluating the likelihood only requires a single application of the Kalman filter. 

As the observation densities $g(z_{t}|s_{t})$ do not depend on $\theta$, their derivatives do not appear in the above estimate. However, when using \gls{eis} to determine an optimal proposal, the parameter $\psi = (z, \omega)$ implicitly depends on $\theta$. Accounting for this yields the gradient 
\begin{align}
...
\end{align}
for free. We can then estimate the log-likelihood in \eqref{eq:loglik-hat-bias-reduction} by replacing each occurrence of $\tilde w_{\theta}(X^{i})$ by 
\begin{align}
...
% intialization
As the procedure to evaluate the likelihood by importance sampling becomes expensive as the dimension of the model increases, \citep{Durbin1997Monte} recommend finding an initial value $\hat \theta_{0}$ by maximizing a deterministic version of \eqref{eq:loglik-hat-standard}. For this, denote by $s^{\ast}$ the mode of the linear signal, conditional on the pseudo-observations $z$. As $S$ follows a multivariate Gaussian, $s^{\ast}$ is also the mean which can be computed efficiently by the Kalman or signal-smoother. Approximating the conditional expectation in \eqref{eq:loglik-pgssm-exact} by $w_{\theta}(s^{\ast})$ then yields 
\begin{align}
...
\begin{algorithm}
    \begin{algorithmic}[1]
        ...
        \Function{estimate\_loglik}{$\theta$}
            \State obtain \gls{la} of the \gls{pgssm} for $\theta$ \Comment{\Cref{alg:la}}
            \State obtain \gls{eis} proposal $\G_{(z,\Omega)}$ \Comment{\Cref{alg:eis}, \gls{la} as initial values}
            ...
            \State calculate antithetic variables $\tilde{S^{i}},\check{S^{i}}, \breve{{S^{i}}}$ \Comment{\eqref{eq:antithetic-location,eq:antithetic-scale,eq:antithetic-scale-location}}
            \State set $\tilde w_{\theta}^i = \frac{1}{4} \left( \tilde w_{\theta} (X^{i}) + \tilde w_{\theta}(\tilde{X^{i}}) + \tilde w_{\theta}(\check{X^{i}}) + \tilde w_{\theta}(\breve{{X^{i}}}) \right)$ \eqref{eq:antithetic-weights}
            ...
            \State \textbf{return} $\verywidehat{\log p_{\theta}(y)}$ \Comment{\eqref{eq:loglik-hat-bias-reduction}}
        \EndFunction
        \Statex
