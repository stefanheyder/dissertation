...
\begin{example}[univariate Gaussian proposal, $\sigma^{2}$ fixed]
    ...
    \begin{figure}
        ...
        \caption{Targets used for the \acrshort{cem} and \acrshort{eis} in \Cref{ex:univ-gaussian-s2-fixed} and \Cref{ex:univ-gaussian-s2-fixed}. In addition to the standard normal distribution, we use location and scale mixtures. The location mixture is chosen for its bimodality for the smaller two values of $\omega^{2}$, while the scale mixture is chosen for its different tail behavior compared to the behavior at the mode.}
        \label{fig:targets}
    \end{figure}
...
Finally, we compare the \acrshort{cem} and \acrshort{eis} in a situation closer to the applications in \Cref{cha:analysis_of_selected_models}. 
\begin{example}[Growth factor \acrshort{ssm}]
    ...
    \begin{enumerate}
        ...
        \item $M$ times, using different seeds, perform the \acrshort{cem} (\Cref{alg:cem-markov-proposal-fast}) to obtain proposals $\G^{1}_{\text{CEM}}, \dots, \G^{M}_{\text{CEM}}$, using the \acrshort{la} as a starting point.
        \item $M$ times, using different seeds, perform \acrshort{eis} (\Cref{alg:eis}) to obtain proposals $\G^{1}_{\text{EIS}}, \dots, \G^{M}_{\text{EIS}}$, using the \acrshort{la} as a starting point. 
    \end{enumerate}
    
    Unfortunately, there is no straightforward way to define the relative efficiencies in this scenario. First, we are in the same scenario as in \Cref{ex:univ-gaussian-mu-fixed}: \acrshort{eis} and the \acrshort{cem} estimate different parameters. Additionally, the parameters estimated by the optimal importance sampling methods are now multivariate. 
...
Let us also return to the \acrshort{ssm} from \Cref{ex:growth_factor_ssm_are}. We use a similar simulation routine to estimate efficiency factors:
...
\begin{enumerate}
    ...
    \item Obtain $\G_{\text{EIS}}$ by \acrshort{eis} (\Cref{alg:eis}), using the \acrshort{la} as a starting point. 
...
\begin{figure}
    ...
    \caption{Boxplots of efficiency factors for the \acrshort{ssm} from \Cref{ex:growth_factor_ssm_are}. Note that the $x$-axis is not to scale, presenting only the values that $n$ takes in the simulations.} 
