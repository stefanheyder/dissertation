...
\subsection{Relative efficiencies of finite-sample and asymptotic variances}
...
\begin{example}[univariate Gaussian proposal, $\sigma^{2}$ fixed]
    ...
    \end{align*}
    where 
    $$\gamma = \cov_{\P} \left( (\log p - \log h - \P \left[ \log p - \log h \right]) \id \right) = \P \left[ \id^{2} \left( \log p - \log h - \P\left[\log p - \log h\right] \right)^{2} \right],$$
    by symmetry of $\log p$ and $\log h$. 
    ...
    $$
        \log p(x) - \log h(x) = \underbrace{\frac{1}{2}(\log \sigma^{2} - \log \tau^{2})}_{a} + \underbrace{\frac{x^{2}}{2}\left(\frac{1}{\sigma^{2}} - \frac{1}{\tau^{2}} \right)}_{b x^{2}}
    $$ 
    and so $\log p - \log h - \P \left[ \log p - \log h \right] = b \id ^{2} - b\tau^{2}$, thus
    \begin{align*}
        \gamma &= \P \left[ \id^{2} \left( b \id^{2} - b \tau^{2}\right)^{2} \right] \\
            &= b^{2}\P \left[\id^{6} - 2\id^{4}\tau^{2} + \id^{2}\tau^{4}   \right] = b^{2} (15 \tau^{6} - 6 \tau^{6} + \tau^{6}) = 10 b^{2} \tau^{6}.
    \end{align*}
...
\end{example}
\vspace{16pt}
Note that for fixed $\sigma^{2}$ the asymptotic variance of the \gls{cem} $V_{\ce}$ is the same in all of the examples considered, as we sample directly from the tractable $\P$, so $V_{\ce}$ only depends on $\P$ through its second moment $\tau^{2}$. 
The asymptotic variance of \gls{eis} however depends on both $\tau^{2}$ and $\gamma$, which depends on higher order moments of $\P$.
