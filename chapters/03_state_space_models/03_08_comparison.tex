...
\begin{example}[univariate Gaussian proposal, $\sigma^{2}$ fixed]
    ...
    \begin{align*}
        V_{\eis} &= 10  \left( \tau^{2} - \sigma^2 \right)^{2}
    \end{align*}
    ...
    To quantify uncertainty in these asymptotic relative efficiencies, we perform the following bootstrap procedure to estimate standard errors of the asymptotic variances, which we sketch for the \acrshort{cem}: 
    \begin{enumerate}
...
\begin{example}[Growth factor \gls{ssm}]
    ...
    We set the number of iterations of the \gls{cem} and \gls{eis} to $100$, which, in our experience, suffices to determine whether the numerical scheme converges or diverges. We use $M=10$ samples to obtain the covariance matrices. For both methods we use $N = 10\,000$ samples for estimation.  
    ...
    \begin{figure}
        ...
        \caption{Boxplots of asymptotic relative efficiencies of \gls{eis} and the \gls{cem} for \Cref{ex:growth_factor_ssm_are}. Note the log-scale of the $y$-axis. Values smaller than $1$ indicate that the \gls{cem} has smaller asymptotic variance, while values larger than $1$ indicate that \gls{eis} has smaller asymptotic variance than the \gls{cem}.}
        \label{fig:ssm_comparison_asymptotic_variance}
    \end{figure}

    We show the resulting asymptotic relative efficiencies in \Cref{fig:ssm_comparison_asymptotic_variance}. For small values of $n$, the \gls{cem} actually outperforms \gls{eis} in terms of asymptotic variance, but starting with $n = 5$, \gls{eis} begins to vastly outperform the \gls{cem}. 
...
\begin{figure}
    ...
    \caption{Efficiency factors of proposals for \Cref{ex:univ-gaussian-s2-fixed} in dependence of $\sigma^{2}$. The efficiency factor is the same for the \gls{cem} and \gls{eis}, as the optimal $\mu$ is always $0$.}
    \label{fig:rho_mu}
...
\begin{figure}
    ...
    \caption{Efficiency factors of the \gls{cem} and \gls{eis} for example \Cref{ex:univ-gaussian-mu-fixed}. The \textbf{left-hand} (\textbf{middle}) figure shows the efficiency factor (in \%) for the \gls{cem} (\gls{eis}). The \textbf{right-hand} figure shows the relative efficiency factor of the \gls{cem} vs. \gls{eis}, with values greater than $1$ indicating that the efficiency factor of the \gls{cem} is bigger than that of \gls{eis}.} 
    \label{fig:rho}
\end{figure} 
...
Let us also return to the \gls{ssm} from \Cref{ex:growth_factor_ssm_are}. We use a similar simulation routine to estimate efficiency factors:
...
\begin{enumerate}
    ...
    \item Draw $N = 10\,000$ samples from each proposal and estimate the efficiency factor by  \eqref{eq:efficency_factor}.
\end{enumerate}
...
\begin{figure}
    ...
    \caption{Boxplots of efficiency factors for the \gls{ssm} from \Cref{ex:growth_factor_ssm_are}. Note that the $x$-axis is not to scale, presenting only the values that $n$ takes in the simulations.} 
    \label{fig:ssm_comparison_efficiency_factor}
\end{figure} 

Again, we repeat this procedure $10$ times for varying levels of $n$ and use $100$ iterations of for all three methods, as well as $10\,000$ samples to estimate the optimal proposal.
The resulting efficiency factors are presented in \Cref{fig:ssm_comparison_efficiency_factor}. As expected, the efficiency factor declines as $n$ grows for all methods. For small $n$, both \gls{eis} and the \gls{cem} produce better proposals --- as measured by the efficiency factor --- than the \gls{la}. There are some issues for the \gls{la} and \gls{eis} for $n=1$, which we believe is due to the initialization scheme that is optimized with multiple observations in mind. From our experience, it is also the case that the optimal parameters for \gls{eis} are close to the optimal parameters produced by the \gls{la} - which may explain the bad performance of \gls{eis} in this case. 

Up to $n=20$, the \gls{cem} produces adequate proposals, however, with starting at $n=50$, the performance deteriorates. Additionally, we had already seen in the previous section, that the asymptotic variance of the \gls{cem} grows much more rapidly than that of \gls{eis}. 
