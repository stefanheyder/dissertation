...
\begin{example}[univariate Gaussian proposal, $\sigma^{2}$ fixed]
    ...
    \begin{align*}
        V_{\text{EIS}} &= \cov_{\P}(T)^{-1} \cov_{\P} \left( (\log w_{\psi_{\eis}}) (T - \P[T]) \right)\cov_{\P}(T)^{-1} \\
        &= \frac{\sigma^{4}}{\tau^{4}} \cov_{\P} \left( (\log p - \log g_{\peis}) \frac{\id}{\sigma}\right) \\
        &= \frac{\sigma^{2}}{\tau^{4}} \cov_{\P} \left( (\log p - \peis T + \frac{\peis^{2}}{2} - \log h) \id \right) \\
        &= \frac{\sigma^{2}}{\tau^{4}} \cov_{\P} \left( (\log p - \log h) \id \right) \\
        &= \frac{\sigma^{2}}{\tau^{4}} \gamma
    \end{align*}
    where 
    $$\gamma = \cov_{\P} \left( (\log p - \log h) \id \right) = \P \left[ \id^{2} \left( \log p - \log h \right)^{2} \right],$$
    ...
    $$
        \log p(x) - \log h(x) = \frac{1}{2}(\log \tau^{2} - \log \sigma^{2}) + \frac{x^{2}}{2}\left(\frac{1}{\tau^{2}} - \frac{1}{\sigma^{2}} \right)
    $$ 
    so 
    \begin{align*}
        \gamma &= \P \left[ \id^{2} \left( \log p - \log h \right)^{2} \right] \\
            &= 
    \end{align*}
    
    
    and $V_{\eis}$ reduces to
    \begin{align*}
        V_{\eis} &= \P \left[ \id   \right]\\
        &=\frac{5}{2} \left( \frac{\tau^{2}}{\sigma^{2}} - 1 \right)^{2} \frac{\sigma^{2}}{\tau^{2}} = \frac{5}{2} \frac{\left( V_{\ce} - 1\right)^{2}}{V_{\ce}}
    \end{align*}
