...
Time and space complexity of a single iteration of each method allow us to compare the theoretical computational resources required to apply the method in practice and can be used to inform our choice, if there is no great difference in performance of the achieved importance sampling proposals.
...
\begin{example}[Failure of \gls{la}]
  ...
  For $\varepsilon$ small enough, the variance of the \gls{la} will be smaller than $\frac{1}{2\varepsilon^{2}}$, whence the second moment of the weights is infinite, by \Cref{lem:gaussian_proposal_factor_2} and importance sampling with $\G_{\text{LA}}$ is inconsistent.
