...
\end{align}
a weighted linear least squares problem. The rationale for centering $T$ under $\P$ will become apparent in \Cref{thm:eis-clt}.
...
We note that this is a convex objective function in $\psi$ which, similar to the \acrshort{cem}, will be very useful to derive asymptotics later on. For now, we begin with studying the existence and uniqueness of $\peis$ similar to \Cref{prop:cem_exponential_families}.
...
Again, if $\G_{\psi}, \psi \in \Psi$ form an exponential family with natural parameter $\psi$, this optimization problem turns into a weighted least squares problem. As $\P[T]$ is not known, we use other unnormalised log-weights $\log\tilde w_{\psi} = \log p - \psi^{T}T$ instead.
...
\begin{proof}
    ...
    $$
        H_{\psi'} b(x, \psi') = -\begin{pmatrix}
            T(x)T(x)^{T} & T(x) \\
            T(x)^{T} &  1
        \end{pmatrix} = -\begin{pmatrix} T(x)^{T} & 1 \end{pmatrix} \begin{pmatrix} T(x)^{T}& 1 \end{pmatrix}^{T},
    $$
...
\begin{theorem}[\acrshort{clt} for $\hpeis$]
    ...
    $$
    M = \cov_{\G} \left( w \left( \log p - \peis^{T}(T - \P[T]) - \lambda_{\eis} \right)(T - \P[T]) \right).
    $$
\end{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    where $\psi' = (\psi, \lambda) \in \Psi \times\R$ and $T'(x) = \begin{pmatrix} (T(x) - \P[T]) & 1 \end{pmatrix}$. For $\psi \in \Psi$ the map $(\psi, \lambda) \to \P [b(\cdot, (\psi, \lambda))]$ is differentiable with gradient 
    $$
