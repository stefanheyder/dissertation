...
\begin{tcolorbox}[title={Contributions}]
    ...
    While application of \acrshort{eis} to importance sampling in \acrshortpl{ssm} is well known, using the \acrshort{cem} for the same task is not. Thus, the following are the new contributions of this section:
    \begin{itemize}
    ...
    \begin{itemize}
        \item The discussion of calculating the gradient, \eqref{eq:gradient_infeasible}, for the \acrshort{la} / \acrshort{eis}.
...
Let us start from a very general definition of an \acrshort{ssm}.
...
\begin{figure}
    ...
    \caption{Dependency structure in an \acrshort{ssm} as given by \Cref{def:ssm}. The dependencies between observations $Y_{t-1}$ (indicated by dotted arrows) are usually not part of the standard definition of an \acrshort{ssm}, but can be incorporated in a straightforward manner.}
