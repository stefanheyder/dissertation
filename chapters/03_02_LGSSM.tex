...
\begin{definition}[\gls{glssm}]
    \label{def:glssm}
    A \acrfull{glssm} is a state space model \todoTH{proof this later} where states obey the transition equation
    \begin{align}
...
Notice that the Kalman filter calculates the likelihood \todoTH{more explicit} $p(y)$ while filtering --- this is possible because of the dependency structure of the state space model --- this makes inference via maximum likelihood possible in \gls{glssm}s.
