...
$$
which suggests to estimate $\zeta$ by simple Monte-Carlo integration: $$\hat \zeta = \frac 1 N \sum_{i=1}^{N} v(X^{i})$$ for $X^{i} \iid \G$, $i = 1, \dots, N$. For this procedure to work, we require that $\hat \zeta$ fulfills a law of large numbers, so we will want $v \in L^{2}(\G)$, where $L^{p}(\nu)$ is the space of $p$-times $\nu$-integrable functions for a measure $\nu$. The i.i.d. assumption could also be dropped, e.g. when we employ antithetic variables, see e.g. \citep[Section 5.3]{Ripley2009Stochastic}.

Here we call $\hat \zeta$ the importance sampling estimate of $\zeta$. 
...
\begin{proposition}[{\citep[Proposition 8.2]{Chopin2020Introduction}}][minimum MSE proposal]
...
\end{align}
where $w = \frac{\d\P}{\d\G}$, as a particle approximation of $\P$, in the sense that for sufficiently well behaved test functions $f$, as $N \to \infty$ $\hat\P_{N}[f]\to \P [f]$. $w_{i} = w(X_{i})$ is called the importance sampling weight of the $i$-th sample.

To perform importance sampling one must be able to evaluate $w$. In the context of \acrshortpl{pgssm} this is usually not possible: if $\P$ is an intractable conditional distribution then the integration constant of its density is not analytically available.
Still, we can usually evaluate the weights up to a constant, i.e. $w(x) \propto \frac{\d \P}{\d \G}(x)$ is available. The missing constant is then $\int w(x) \d \G$ which is itself amenable to importance sampling: we may estimate it by $\sum_{i = 1}^N w_i$.
...
\begin{theorem}[{\citep[Theorem 1.1]{Chatterjee2018Sample}}]
    \todoTH{fix this Thm}
...
To judge whether importance sampling has converged, several criteria are discussed in the literature. The classic \gls{ess}\citep{Kong1994Sequential} 
$$
\text{ESS} = \frac{1}{\sum_{i = 1}^N W^{2}_{i}}
$$
arises from an analysis of the asymptotic efficiency of importance sampling estimates and is easy to interpret. Assessing convergence through the empirical variance of $\hat \zeta$ is, while seemingly natural, flawed \citep{Chatterjee2018Sample} \todoTH{more explicit \& intructive} and should be avoided. As a remedy \citep{Chatterjee2018Sample} suggest the heuristic $q_{N} = \E Q_{N}$ where
$$
