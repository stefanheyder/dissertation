...
Let us consider the implications of \Cref{thm:chatterje2018Thm1} by devising heuristics to decide when $\G$ is a good proposal for fixed sample size $N$.
...
$$
If, additionally, $\log w(Z)$ has finite variance, Tchebyscheff's inequality additionally yields 
$$
...
\begin{lemma}
    ...
    $$
    \mu = -\log \left( 1 + \exp(\delta) \frac{\sigma^{2}}{2} \right).
    $$
    In particular, 
    $$
    \mu \geq  - \log \left( 1 + \exp(a) \frac{\sigma^{2}}{2} \right).
    $$
\end{lemma}

\begin{proof}
    As $U$ is bounded, all involved expectations exist and are finite. We perform a first-order Taylor expansion of $\exp(U)$, where the random variable $\xi$ is between $U$ and $\mu$:
    $$
    1 = \exp(\mu)\mathbb E \exp (U - \mu) = \exp (\mu) \left( 1 + \mathbb E (U - \mu) + \mathbb E\left(\frac{(Z-\mu)^{2}}{2} \exp(\xi)\right) \right).
    $$
    Rearranging terms, we obtain 
    $$
    \mu = - \log \left( 1 + \mathbb E\left(\frac{(Z-\mu)^{2}}{2} \exp(\xi)\right) \right),
    $$
    and as $\xi \in [a, b]$ we have 
    $$
    \frac{\sigma^{2}}{2} \exp a \leq \mathbb E\left(\frac{(Z-\mu)^{2}}{2} \exp(\xi)\right) \leq \frac{\sigma^{2}}{2} \exp b.
    $$
\end{proof}
...
\begin{corollary}
    ...
    If $\log w \in L^{2}(\P)$ with variance $\sigma^{2} = \P [(\log w - L)^2]$, then 
    $$
    \Dkl{\P}{\G} \leq \log \left(1 + \exp(b) \frac{\sigma^{2}}{2} \right).
    $$
\end{corollary}
\todo{add a comment on the second equation in Thm: min KL also implies that for smaller N we dont run into problems }
...
In practice we will want to judge whether importance sampling has converged, and there are several criteria available in the literature. The classic \gls{ess}\citep{Kong1994Sequential} 
$$
