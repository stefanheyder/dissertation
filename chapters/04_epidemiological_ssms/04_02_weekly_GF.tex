...
Before we detail our proposed model, let us give an overview over similar approaches to modeling epidemics on a regional level in the literature. Each such approach will come with a general approach, i.e. the statistical models that are fit to epidemiological data, usually case counts, but also hospitalizations or deaths. Additionally, these models will differ in how incorporate spatial data as well as the granularity of the regions considered.
...
The spatio-temporal models account for regional dependencies by adding spatially correlated random effects. \citep{Kline2022Bayesian} construct a Bayesian spatio-temporal model to perform nowcasting for COVID-19 cases in counties in Ohio, USA. They include a spatially correlated random effect to account for dependencies between neighboring counties, similar to our approach. However, we differ from their approach by incorporating commuter data to model dependencies between all regions, not only neighboring ones. \citep{DeNicola2022Regional,Schneble2021Nowcasting} perform Bayesian nowcasting for German counties using splines for temporal and spatial effects. 

Further approaches based on classical time series analysis fit models to individual regions as a pre-processing step to infer epidemiological indicators such as the reproduction number or growth rate, then use spatial models to infer about regional effects. These methods suffer from the major disadvantage that they do not account for spatial dependencies when fitting the initial models, which may lead to biased estimates as well as neglected uncertainty. 
\citep{Ives2021Estimating} estimate the reproduction number for counties in the United States individually using a local-level state space model and use a spatial model to smooth these estimates across neighboring counties. 
\todo{continue lit review.}
%% classical ts models
