...
We will come back to this literature once we have developed a new model to predict \acrshort{c19} incidences on the regional level, using an \acrshort{ssm} based approach. 
...
In conclusion, our model addresses several gaps identified in our short literature review at the start of this section, by providing a flexible, yet interpretable framework for modeling the spatio-temporal spread of an infectious disease such as \acrshort{c19}. Compared to network-based \citep{Armbruster2024Networkbased} and spatio-temporal approaches \citep{Kline2022Bayesian}, our method achieves a balance between complexity, interpretability and predictive performance. While the aforementioned limitations remain, these allow for promising avenues for future research, by, including dynamic mobility modeling and real-time forecast evaluation, e.g. in the \acrshort{ecdc}'s RespiCast Hub \citep{ECDCRespiCast}. Overall, we believe this model to provide a valuable contribution to the toolkit of epidemic modeling at the regional level, with potential application beyond the current context. 