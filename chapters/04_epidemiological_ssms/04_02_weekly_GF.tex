...
\begin{itemize}
    ...
    \item The \texttt{itww} model makes predictions based on a county-level reproduction number model, with forecasts based on repeated simulations from the renewal equatino \Cref{eq:renewal-model}. See \citep{Heyder2022Regional} for details.
\end{itemize}
...
% ITWW just beats the baseline
% SSM somewhere between baseline and ensemble
% replicating ensemble performance not sensible
