...
\subsection{Context}
Retrospective analysis of the reported cases is one of the main tasks of epidemiological monitoring, see \Cref{sec:objectives}. For this analysis, it is crucial to have the finest temporal resolution possible, as we want, e.g., to link the dates on which \acrshortpl{npi} were enforced to the growth factors on, or surrounding these dates. However, as we have observed in \Cref{sec:data}, the data at our hand are contaminated by the reporting process, most notably the weekday effects, reporting delays, and reporting artifacts, e.g. due to public holidays.
While we can make the influence of these effects small by aggregating data to the weekly level, see \Cref{sec:regional_growth_factor_model}, modeling on the daily level facilitates better retrospective analyses and as such it is the goal of this section.

Here, we use the \acrshort{rki} case incidence data discussed in \Cref{sec:data}. As we have seen in \Cref{fig:reporting_delays_cases} \textbf{A} and \Cref{fig:survival_function_rep_tri_incidences}, most delays are shorter than $4$ days. Thus, ignoring any cases reported with longer delays, we get for any reporting date $t$ four observations, say 
$$
...
%perform short-term forecasts for the case incidence in Germany, including prediction intervals. To assess the performance of our method, we compare the results to those produced in the \acrfull{ecdc}'s ForecastHub \citep{Sherratt2022Predictive}.
...
$$
for $\varepsilon_{t + 1, W} \sim \mathcal N(0, \sigma^{2}_{W})$, as described in \Cref{sec:modelling_epidemiological_dessiderata_with_state_space_models} \todo{explicit this here}. Finally, to model the reporting delay probabilities $p_{t,\tau}$, $\tau = 1,2,3,4$, we parameterize them by log ratios
\begin{align*}
