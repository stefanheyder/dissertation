...
\end{pmatrix}
$$
has independent marginals, and fixing an initial distribution of $X_{0}$ fully specifies an \gls{pgssm} for the joint distribution of $(X,Y)$. For the initial distribution we use 
$$
...
\begin{table}
    ...
    \caption{Standard deviations for the models' showcase determined either by hand, by the initial search or by maximum likelihood estimation described in \Cref{sec:maximum_likelihood_estimation}. The difference between the initial search and the \gls{mle} is negligible and is not visible for the precision shown here.}
