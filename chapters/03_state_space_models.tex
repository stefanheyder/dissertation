...
\begin{notation}[central limit theorem for Z-estimators]
    ...
    \end{itemize}
    The naming of $B$ and $M$ stems from the sandwich estimator \todo{cite white 1982}, where $B$ is the Jacobian of the estimating equations $\G^{\ast}f_{psi} = 0$ and $M$ is the covariance matrix of $f_{\psi}$ under $\G^{\ast}$. 
\end{notation}
...
\begin{theorem}[asymptotic normality of $\hat \psi_{\text{CE}}$]
    ...
    Let $\G_{\psi}$ form a natural exponential family with densities $g_{\psi}(x) = \frac{h(x)}{Z(\psi)} \exp \left( \psi^{T}T(x)\right) $ w.r.t. $\mu$. Let $\G, \G^{\ast}$ be two other probability measures such that $\G \ll \G^{\ast}$ and let $W = \frac{\d \G^{\ast}}{\d \G}$ be the normalized importance sampling weights. 
    Suppose further that 
    \begin{enumerate}[label={\bfseries(A{\arabic*})}]
        \item\label{it:exist-unique-psice} $G_{\hpce} T = \hat\G_{N} T$ $\mu$-a.s. has a unique solution $\hpce$,
        \item\label{it:zdot-ll} $\psi \mapsto \nabla_\psi \log Z(\psi)$ is locally Lipschitz around $\psi_{\text{CE}}$,
        \item\label{it:w-t-wt-L2} $W,T$ and $WT$ possess finite second moments  w.r.t. $\G$ and
        \item\label{it:FI-psd} the Fisher information $I(\psi_{\text{CE}})$ is positive definite.
...
\begin{proof} We check that the conditions of the central limit theorem for Z-estimators (\Cref{thm:clt_z_est_vdv}) are fulfilled. This proof uses the notation established in \Cref{not:notation-clt}. Consider the estimating equations for $\pce$ 
    ...
    By \ref{it:exist-unique-psice} $\hat G_{N} f_{\hpce} = 0$ $\mu$-a.s., so it remains to show that $\hpce \to \pce$  in probability, which is implied by \cref{thm:ce-consistent} \todo{check}.
...
$$
\left( \hat\lambda', \hpeis \right) = \left(\mathbf X^{T} \mathbf X\right)^{-1}\mathbf X^{T} \left( \log p(X^{1}), \dots, \log p(X^{N}) \right)
$$
...
\begin{theorem}[asymptotic normality of $\hpeis$ I]
    Let $\G_{\psi}$ form a natural exponential family with densities $g_{\psi}(x) = \frac{h(x)}{Z(\psi)} \exp \left( \psi^{T}T(x) \right)$ w.r.t. $\mu$. Let $\G, \G^{\ast}$ be two other probability measures such taht $\G \ll \G^{\ast}$ and let $W = \frac{\d\G^{\ast}}{\d\G}$ be the normalized importance sampling weights. 
    Assume $\lambda(\psi) = \G^{\ast} \log w_{\psi}$ is known and the following conditions hold:
    \todo{faster convergence if $\G^{\ast}$ is from same EF?}
    \begin{enumerate}[label={\bfseries(B\arabic*)}]
        \item ...
        \item\label{it:eis-dkl-dkl-to-base-finite} $\Dkl{\G^{\ast}}{\G_{\psi}} < \infty$ for all $\psi$ \todo{prob. suffices locally}
        \item\label{it:eis-T-l2} $T$ is square integrable w.r.t. $\G^{\ast}$.
        \item\label{it:eis-dkl-regularity} regularity conditions
    \end{enumerate}
    Then, as $N$ goes to $\infty$,
    $$
    \sqrt{N} \left( \hpeis - \peis \right) \Dto \mathcal N(0, V_{\eis})
    $$
    where 
    $$
    ...
    with
    \begin{align*}
        B_{\eis} &= \left( \cov_{\G^{\ast}} T \right)^{-1},\\
        M_{\eis}%&= \G \left(W^{2} \left( \log w_{\psi} - \lambda(\psi) \right)^{2} \left( T - \G^{\ast} T \right)\left( T - \G^{\ast} T \right)^{T}\right)\\
        &= \cov_{\G} \left( W \log w_{\psi}, W T \right).
    \end{align*}
\end{theorem}
...
\begin{proof}
    ...
    % verify conditions
    As $\lambda (\psi) = \G^{\ast} (\log g^{\ast} - \log g_{\psi})$ its gradient is $\nabla_{\psi} \lambda (\psi) = -\G^{\ast} T + \dot z(\psi)$, so the estimating equations for $\peis$ are \todo{regularity for $\nabla \lambda$}
    $$
    x \mapsto f_{\psi}(x) = \nabla_{\psi} -\frac{1}{2}w(x)\left(\log w_{\psi} (x) - \lambda(\psi) \right)^{2} = w(x)\left( \log w_{\psi}(x) - \lambda (\psi) \right) \left( T(x) - \G^{\ast} T \right).
    $$
    % LL
    For $\psi_{1}, \psi_{2}\in\Psi$ the Lipschitz condition \cref{eq:clt-vdv-local-lipschitz} are fulfilled, as 
    \begin{align*}
    \lVert f_{\psi_{1}}(x) - f_{\psi_{2}}(x)\rVert &= w(x)\lvert \log w_{\psi_{1}}(x) - \lambda(\psi_{1}) - (\log w_{\psi_{2}}(x)  - \lambda(\psi_{2}))\rvert\cdot\lVert T(x) - \G^{\ast}T\rVert   \\
        &= w(x) \lvert \left( (\psi_{1} - \psi_{2})^{T} \left( T(x) - \G^{\ast} T \right)  \right)\rvert \lVert T(x) - \G^{\ast} \rVert  \\
        &\leq w(x) \lVert T(x) - \G^{\ast} T\rVert^{2} \lVert \psi_{1} - \psi_{2} \rVert,
    \end{align*}
    and $\G w \lVert T - \G^{\ast}T\rVert^{2} = \G^{\ast} \lVert T - \G^{\ast}T\rVert^{2} < \infty$.
    % Gf_psi0 < infy
    $\G f_{\peis} = \G^{\ast} $
