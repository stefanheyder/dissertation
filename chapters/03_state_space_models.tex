...
\begin{theorem}[consistencty of $\hat\psi_{\text{CE}}$]
    ...
    Then, as $N \to \infty$, $\hat\psi_{\text{CE}}$ goes to $\psi_{\text{CE}}$ in probability.
\end{theorem}
% proof: appeal to vdV, pd ensures that M(\theta) is convex, so global maximum unique and well separated
If $\G_\psi$ belongs to an exponential family, the assumptions may be weakened a bit:
\begin{theorem}[consistency of $\hat \psi_{\text{CE}}$ in exponential families]
