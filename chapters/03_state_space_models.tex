...
\acrfull{glssm} are the working horses of most methods used in this thesis because they are analytically tractable and computationally efficient. Indeed for fixed dimension of states $m$ and observations $p$ the runtime of algorithms that we consider in this thesis is $\mathcal O(n)$.

\begin{definition}[\acrfull{glssm}]
    A \acrshort{glssm} is a state space model where states obey the transition equation
    \begin{align}
...
The defining feature of a \acrshort{glssm} is that its joint distribution is Gaussian.

\begin{lemma}
    A state space model can be written as a \acrshort{glssm} if and only if its joint distribution is Gaussian.
\end{lemma}
\begin{proof}
...
\begin{algorithm}
    ...
    \KwIn{observations $y = (y_{0}, \dots, y_{n})$, \acrshort{glssm}}
...
Notice that the Kalman filter calculates the likelihood $p(y)$ while filtering --- this is possible because of the dependency structure of the state space model --- this makes inference via maximum likelihood possible in \acrshort{glssm}s.
...
The modeling capacity of \acrshort{glssm} is, however, limited: most interesting phenomena follow neither linear dynamics nor are well modeled by a Gaussian distribution.
Nevertheless, linearization of non-linear dynamics suggests that \acrshort{glssm}s may have some use as approximations to these more complicated phenomena, provided they are sufficiently close to Gaussian models, e.g. unimodal and without heavy tails. 
...
Y|X \sim Y|BX \sim \the smathcal p(y|s)
\end{align*}
and the state space formulation allows for efficient computation of, e.g., $p(y|x)$.
To perform importance sampling for the smoothing distribution $p(x|y)$ we want to have approximations that are close, but also depend on few parameters, ideally only $\mathcal O(n)$ many. 
...
If, additionally, the observations $y_{t}$ are conditionally independent given $x_{t}$, i.e. if $p(y_{t}|s_{t}) &= \prod_{i = 1}^{p} p(y_{t}^{i}|s_{t}^i)$, then $\Omega$ is a diagonal matrix with only $\mathcal O(n \dot m)$ many non-zero entries. 
