...
$$
Given $\E T(X)$, this system of equations can, in many cases, be solved analytically or by gradient descent algorithms.

While $\E T(X)$ is usually not available, it is itself amenable to importance sampling. Given a proposal $\G$ we may estimate $\E T(X)$ by $\hat\G_N T = \sum_{i = 1}^{N} W^{i} T(X^{i})$ for $X_{1}, \dots, X_{N} \iid \G$ and auto-normalized importance sampling weights $W^{i}$ and in turn estimate $\psi_{\text{CE}}$ by $\hat \psi_{\text{CE}}$ solving
$$
\frac{\nabla_{\psi}Z(\hat\psi_{\text{CE}})}{Z(\hat\psi_{\text{CE}})} = - \frac{\hat G_N T}{\hat G_N w}.
$$
...
\begin{theorem}[asymptotic normality of $\hat \psi_{\text{CE}}$]
    ...
    Then
    $$
        \sqrt{N} \left(\hat\psi_\text{CE} - \psi_{\text{CE}}\right) \to \mathcal N \left(0, V\right)
    $$
    where 
    $$
    V = - \frac{1}{(\G w)^2}I(\psi_{\text{CE}})^{-1}  \text{Cov}_{\G} \left( w (T - \nabla_{\psi_{\text{CE}}}\log Z(\psi_{\text{CE}})) \right) I(\psi_{\text{CE}})^{-1}.
    $$
    Moreover $\G (w(T - \nabla_{\psi}\log Z(\psi_{\text{CE}}))) = 0$, so we may estimate $V$ consistently by 
    $$
    \hat V = I(\hat\psi_{\text{CE}})^{-1}  \frac{\sum_{i = 1}^N w^{2}_{i} \left(T(X^{i}) - \nabla_{\psi}\log Z(\hat\psi_{\text{CE}}) \right)\left(T(X^{i}) - \nabla_{\psi}\log Z(\hat\psi_{\text{CE}})\right)^{T}}{\left(\sum_{i = 1}^N w_{i}\right)^{2}} I(\hat\psi_{\text{CE}})^{-1}.
    $$
\end{theorem}
...
\end{align*}
which may be used to derive asymptotics for $\hpeis$. Similar to the \gls{cem} we restrict ourselves here to exponential families where $\log g_{\psi}(x) = \psi^{T}T(x) - \log Z(\psi) + \log h(x)$. In this setting, $\hpeis$ becomes a weighted linear least squares estimator, which can be calculated analytically.
