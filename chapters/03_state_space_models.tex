\glsresetall
\newcommand{\psice}{{\ensuremath{\psi_{\text{CE}}}}}
...
\begin{theorem}[asymptotic normality of $\hat \psi_{\text{CE}}$]
    ...
    where 
    $$
    V = - \frac{1}{(\G w)^2}I(\psi_{\text{CE}})^{-1}  \text{Cov}_{\G} \left( w (\nabla_{\psi_{\text{CE}}}\log Z(\psi_{\text{CE}}) + T) \right) I(\psi_{\text{CE}})^{-1}.
    $$
    Moreover $\G (w (\nabla_{\psi}\log Z_{\psi_{\text{CE}}} + T)) = 0$, so we may estimate $V$ consistently by 
    $$
    \hat V = \frac{1}{\left(\sum_{i = 1}^N w_{i}\right)^{2}}I(\hat\psi_{\text{CE}})^{-1}  \left( \sum_{i = 1}^N w^{2}_{i} (\nabla_{\psi}\log Z(\hat\psi_{\text{CE}}) + T(X^{i}))(\nabla_{\psi}\log Z(\hat\psi_{\text{CE}}) + T(X^{i}))^{T} \right) I(\hat\psi_{\text{CE}})^{-1}.
    $$
\end{theorem}

\begin{proof}
    We check that the conditions of the central limit theorem for Z-estimators \cite[Theorem 5.21]{VanderVaart2000Asymptotic} are fulfilled. Let $\dot z = \nabla_\psi \log Z(\psi)$ and consider the functions $$x\mapsto f_\psi(x) = \nabla_{\psi} \left(w(x)\log g_{\psi}(x)\right) = w(x) \dot z (\psi) + w(x) T(x)$$ for $\psi \in \Psi$. Then $\left\lVert f_{\psi_1}(x) - f_{\psi_2}(x)\right\rVert = w(x) \left\lVert \dot z (\psi_1) - \dot z(\psi_2)\right\rVert$ where $\G w^2(x) < \infty $ and $\psi \mapsto z(\psi)$ is locally Lipschitz by our assumptions. Thus the local Lipschitz condition required by \cite[Theorem 5.21]{VanderVaart2000Asymptotic} is fulfilled.

    Furthermore, as $wT$ is square integrable w.r.t. $\G$, we have 
    $$
    \G \left\lVert f_\psi \right\rVert^2 \leq \G w^2 \left\lVert \dot z(\psi) \right\rVert ^2  + \G \left\lVert wT\right\rVert^2 < \infty.
    $$
    ...
    $$
    \G \left(f_{\psice}\right) = \P \left( \dot z(\psice) + T \right) = \dot z (\psice) + \P T = 0,
    $$
    by definition of $\hat\psice$.
    By \cite[Theorem 5.21]{VanderVaart2000Asymptotic} the asymptotic covariance matrix is 
    $$
    V(\psice) = \frac{\ddot z(\psice)^{-1}}{\G w} \left(\G f_\psice f_{}\psice^T \right) \frac{\ddot z(\psice^{-1})}{\G w},
    $$
