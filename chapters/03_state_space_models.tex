...
\begin{proposition}[Exponential family of proposals]
    ...
    Suppose $\omega^{2} > 0$.
    In this case the family of conditional distributions \Cref{eq:glssm-proposal} form an exponential family

    \begin{align*}
        g(x|z) & = h(x) \exp \left( \left\langle \psi, T(x) \right\rangle - \log Z (\psi)\right)
    \end{align*}
    where
    \begin{align*}
      \psi &= \left( \psi_{1}, \psi_{2} \right) = \left( \bar\Sigma^{-1} \bar\mu, -\frac{1}{2} \Omega^{-1}\right) \\
      \intertext{and }
      h(x) &= \frac{1}{\sqrt{(2\pi)^{m} \det \Sigma}}\exp \left( - \frac{1}{2} x^{T} \Sigma^{-1}x \right) \\
      \log Z (\psi) &= \frac{1}{2} \left( \log \det \left( I - \Sigma \,\diag \left( 2 \psi_{2} \right)  \right)  + \bar\mu^{T}\bar\Sigma^{-1}\bar\mu \right) \\
      &= \frac{1}{2} \log \det \left(I - \Sigma \, \diag\left(2\psi_{2}\right)\right) + \frac{1}{2} \psi_{1}^{T} \left( \Sigma^{-1} - \diag (2\psi_{2}) \right)  \psi_{1}\\
      T(x) &= \left( x, x x ^T \right).
    \end{align*}

    Note that $\psi_{1} = \Sigma ^{-1} \mu + B^{T}\Omega^{-1}z \in \Sigma^{-1}\mu + \operatorname{im} B^{T} \subseteq \R^{m}$ making the exponential family curved if $\operatorname{rank} B < m$.

    If $B$ has full rank $m$, the Fisher information in this model is given by
    $$
    I \left( \psi_{1}, \psi_{2} \right) = \begin{pmatrix}
        \Sigma^{-1} + \Omega^{-1} & - 2 \diag(\psi_{1}) \\
        -2 \diag(\psi_{1}) & -2 \left( \Sigma^{-1} + \Omega^{-1} \right)^{-1} \otimes \left( \Sigma^{-1} + \Omega^{-1} \right)^{-1}
    \end{pmatrix}
    $$
