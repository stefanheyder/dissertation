...
\end{align*}
where $\G^{\ast}$ is either the optimal proposal from \Cref{prop:minimum_MSE_IS} or $\P$. If $\G^{\ast}$ and $\G_{\psi}$ possess densities $g^{\ast}$ and $g_{\psi}$ w.r.t. some common measure $\mu$, the same for all $\psi$, we may rewrite the optimization problem to
\begin{align*}
    \psi_{\text{CE}} &= \argmin_{\psi \in \Psi} \int \log g^{\ast}\log g^{\ast}(x) \d \mu(x) - \int g^{\ast}\log g_{\psi} \d \mu(x) \\
    &= \argmax_{\psi \in \Psi} \int g^{\ast} \log g_{\psi} \d \mu(x).
\end{align*}
This last integral is the cross-entropy between $g^{\ast}$ and $g_{\psi}$.
