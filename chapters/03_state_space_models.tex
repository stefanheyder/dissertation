...
While $\G^{\ast} T$ is usually not available, it is itself amenable to importance sampling. Given a proposal $\G$ we may estimate $\G^{\ast} T$ by $\hat\G_N T = \sum_{i = 1}^{N} W^{i} T(X^{i})$ for $X^{1}, \dots, X^{N} \iid \G$ and auto-normalized importance sampling weights $W^{i}$ and in turn estimate $\psi_{\text{CE}}$ by $\hat \psi_{\text{CE}}$ solving
$$
\G_{\hpce} T = \hat \G_N T.
$$
...
\begin{theorem}[asymptotic variance of Z-estimators, {\cite[Theorem 5.21]{VanderVaart2000Asymptotic}}]
    ...
    $$
    \lVert f_{\psi_{1}}(x) - f_{\psi_{2}}(x)\rVert \leq \dot f(x) \lVert \psi_{1} - \psi_{2}\rVert.
    $$

    Assume that $\G \lVert f_{\psi_{0}}\rVert < \infty$ and that the map $\psi \mapsto \G f_{\psi}$ is differentiable at $\psi_{0}$, with nonsingular derivative matrix $B^{-1}_{\psi_{0}}$. If $\hat\G_{N} f_{\hat\psi_{N}} = o_{P} \left( N^{-\frac{1}{2}} \right)$, and $\hat\psi_{N} \to \psi_{0}$ in probability, then
    $$
...
\end{align}
\todo{more explicit with NEF}
which may be used to derive asymptotics for $\hpeis$. Similar to the \gls{cem} we restrict ourselves here to exponential families where $\log g_{\psi}(x) = \psi^{T}T(x) - \log Z(\psi) + \log h(x)$. In this case \cref{eq:eis-optimal} simplifies to
\begin{align*}
...
In this setting, $\hpeis$ becomes a weighted linear least squares estimator, which can be calculated analytically after reparameterizing to $\psi' = (\lambda - \log Z(\psi),\psi)$ and $T'(x) = \left(1, T(x)^{T}\right)^{T}$:
$$
\hat\psi'_{\text{EIS}} = \left(\mathbf X^{T} \mathbf X\right)^{-1}\mathbf X^{T} \left( \log p(X^{1}), \dots, \log p(X^{N}) \right)
$$
...
\begin{theorem}[CLT for \gls{eis} with known \gls{kld}]
    Assume $\lambda(\psi) = \G^{\ast} \log w_{\psi}$ is know. \todo{+ reg. cond.} Then, as $N$ goes to $\infty$,
    $$
    \sqrt{N} \left( \hpeis - \peis \right) \Dto \mathcal N(0, BMB)
    $$
    where 
    $$
    B = \left(\text{Cov}_{\P} T\right)^{-1}
    $$
    and 
    $$
    M = \G \left(W^{2} \left( \log w_{\psi} - \lambda(\psi) \right)^{2} \left( T - \P T \right)\left( T - \P T \right)^{T}\right).
    $$
\end{theorem}
