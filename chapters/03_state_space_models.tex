...
\begin{align}
    ...
    \eta_{t} &\sim \N \left( 0, \Omega_{t} \right),\\
    \Omega_{t} &= \diag \left( \omega_{t} \right) = \diag \left( \omega_{t,1}, \dots, \omega_{1,p} \right).
\end{align}
where the distribution of $X$ is given by \eqref{eq:glssm_states}, $\psi = \left( z, \omega \right)$ for $z = \left( z_{0}, \dots, z_{n} \right) \in \R^{n \times m}$ and $\omega = \left( \omega_{0}, \dots, \omega_{n} \right) \in \R^{n \times m}$.
...
\end{align}
for $t = 1, \dots, n$. The number of parameters is $(n + 1)\cdot m$ for the means $v_{0}, \dots, v_{n + 1}$, $n \cdot m^{2}$ for the transition matrices $C_{t}$ and $(n + 1) \frac{m (m - 1)}{2}$ for the covariance matrices of innvoations, totalling $\mathcal O(n\cdot m^{2})$ many parameters. 
While this may be considerably more parameters for large state dimension $m$, we will see that finding the optimal parameters for the \gls{cem} can be done analytically \todo{cite relevant section}. 
...
\begin{example}[univariate Gaussian, $\sigma^{2}$ fixed]
    ...
    Note that $T$ is centered under $\P$.
    ...
    As $\sigma^{2} > \frac{\tau^{2}}{2}$ is required for importance sampling to converge for bounded functions, we analyze 

    \begin{figure}
        \input{tikz/normal_are.tex}
        \caption{Relative efficiency $ \frac{V_{\ce}}{V_{\eis}}$ for $\tau^{2}=1$ based on $N = 10\,000$ samples}
    \end{figure}
    
    and we show differrent asymptotics for this relative efficiency in \Cref{tab:comparison-asymptotics}. 
...
\begin{example}[univariate Gaussian, $\mu$ fixed]
    ...
    Consider the same setup as in \Cref{ex:univ-gaussian-s2-fixed} but let $\G_{\psi} = \mathcal N(0, -\frac{1}{2\psi})$ be the single parameter natural exponential family of Gaussians with fixed mean $\mu=0$. Then
    $$
    \log g_{\psi}(x) = \psi T(x) + \frac{1}{2}\log \left( - 2 \psi \right) - \frac{1}{2} \log 2\pi
    $$
    for $T(x) = x^{2}$. Then $I(\psi) = \frac{1}{2\psi^{2}} = 2\sigma^{4}$, $\pce = \sigma^{2}$ and $\cov_{\P} (T) = \nu - \sigma^{4}$ for $\nu = \P \operatorname{id}^{4}$ and $\sigma^{2} = \P \id^{2}$. In total
    \begin{align}
        V_{\ce} &= \frac{1}{4\sigma^{4}} \left( \frac{\nu}{\sigma^{4}} - 1 \right).
    \end{align}

    For \gls{eis} we have 
    \begin{align*}
    \peis &= \left( \cov_{\P} \id^{2} \right)^{-1} \cov_{\P} \left( \id^{2}, \log p \right) \\
        &= \left( \nu - \sigma^{4} \right)^{-1} \underbrace{\int p(x)x^{2}\log p(x)\d x}_{=\gamma},
    \end{align*}
    so $\sigma^{2}_{\eis}= \frac{\nu - \sigma^{4}}{\gamma}$. 
