...
The ideas in this section are based on the ``ILM-prop'' model that Thomas Hotz and I submitted to the German COVID-19 NowcastHub \todo{ref}. However, the model submitted there was based on simple point estimates for the proportion of hospitalisations per reported case, neglecting regularization over time. Here we adapt this model to the \gls{ssm} setting of this thesis and investigate if the increased model complexity results in improved performance.
